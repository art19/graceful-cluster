...
/*
 ...
 - options.willShutdownFunction  - function, provide custom logic performed upon shutdown.
...
GracefulCluster.start = function(options) {
    ...
    var willShutdownFunction = options.willShutdownFunction || (function willShutdown(finishShutdown) { finishShutdown(); });
    ...
    if (cluster.isMaster) {
        ...
        function checkRestartQueue() {
            ...
            if (restartQueue.length > 0 && listeningWorkersCount === workersCount && !currentRestartingPid) {
                ...
                try {
                    ...
                    log(`--> Trying to restart worker, sending process.kill to ${pid}`);
                    process.kill(pid);
        ...
        // Shut down all workers and finally exit the cluster
        function finishShutdown() {
            ...
            for (var id in cluster.workers) {
                ...
                log(`--> Master is sending a SIGTERM to child ${cluster.workers[id].process.pid}`);
        ...
        function startShutdown() {
            log('--> MASTER received a SIGTERM');
            willShutdownFunction(finishShutdown);
        }

        function startInt() {
            log('--> MASTER received a SIGINT');
            willShutdownFunction(finishShutdown);
        }

        process.on('SIGTERM', startShutdown);
        process.on('SIGINT', startInt);
        ...
        cluster.on('exit', function(worker, code, signal) {
            log(`--> Worker ${worker.process.pid} exited with code ${code}. Signal ${signal}`);
            ...
            listeningWorkersCount--;
            if (sigkill) {
                log('--> Master is in SIGKILL mode: checking for workers');
                checkIfNoWorkersAndExit();
                return;
            } else {
                log('--> Master is NOT in SIGKILL mode - Need to bring back up that worker');
            }
            fork();
        });
